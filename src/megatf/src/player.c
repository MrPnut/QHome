...
void player_stand1(  )
{
	...
	} else
	{
		...
		{
			self->s.v.frame = 57;
			//G_centerprint ( self, "                  לבשימח\n" );
			if ( self->current_menu == MENU_DEFAULT/* && ( self->s.v.classname != "player_prop" ) */)
			{
				//G_centerprint ( self, "                  לבשימח\n" );
	...
	if ( self->walkframe == 1 && self->vision == 1 )
...
void player_run(  )
{
	float fstep;		// determinee footstep sound
	fstep = g_random( );
	...
	// footstep sounds
	if ( tf_data.footsteps == 1 && checkbottom( self ) == 1 && self->playerclass != 0 ) {
		if ( self->s.v.velocity[0] > 200 || self->s.v.velocity[1] > 200 || self->s.v.velocity[0] < -200 || self->s.v.velocity[1] < -200 ) {
...
void W_FireMTFSpikes();
void player_mtf_nail2();
void player_mtf_nail1(  )
{
	self->s.v.frame = 103;
	self->s.v.think = ( func_t ) player_mtf_nail2;
	self->s.v.nextthink = g_globalvars.time + 0.1;
...
{
	self->s.v.frame = 104;
	self->s.v.think = ( func_t ) player_mtf_nail1;
	self->s.v.nextthink = g_globalvars.time + 0.1;
...
void player_assaultcannonup1(  )
{
	...
	if ( !self->s.v.button0 || self->s.v.ammo_shells < 1/*self->assault_min_shells*/ || intermission_running )
	{
...
void player_assaultcannon1(  )
{
	...
	if ( !self->s.v.button0 || self->s.v.ammo_shells < 1 || intermission_running )
	{
...
//      gedict_t *tcool;
	...
	if ( self->count >= 15 )
	{
		...
		self->tfstate = self->tfstate - ( self->tfstate & TFSTATE_AIMING );
		TeamFortress_SetSpeed( self );
		if ( self->s.v.ammo_shells <= 1/*self->assault_min_shells*/ || self->s.v.ammo_cells < 4 )
		{
...
void PainSound(  )
{
	...
	{
		sound( self, 2, "player/gib2.wav", 1, 0 );
		return;
	}
			...
			else
			{
				...
				else
				{
					if ( rs == 5 )
						...
						self->s.v.noise = "player/mpain6.wav";
				}
			}
		}
	}
...
void DeathSound(  )
{
	...
	if ( rs == 5 )
		self->s.v.noise = "player/mdeath5.wav";
...
void GibPlayer(  )
{
	...
	if ( g_random(  ) < 0.2 )
		sound( self, 2, "player/gib3.wav", 1, 0 );
	else
		sound( self, 2, "player/gib2.wav", 1, 0 );
}
