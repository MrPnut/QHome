...
void TeamFortress_MOTD(  )
{
	...
	if ( self->motd == 21 )
	{
		...
		rnum = ( int ) g_random();
		// play the megatf join music
		infokey( self, "tf_music", mu, sizeof( mu ) );
		if ( tf_data.tf_music == 1 && strcmp( mu, "off" ) )
		{
			if (rnum < 0.33)
			{
		...
		G_sprint( self, 2, MEGATF_VERSION );
...
//		G_sprint(self, 2, "Official ÍÔÆÌ version\n");
		G_sprint( self, 2, "C code by sd‘ angel for ÍÔÆÌ\n" );
		G_sprint( self, 2, "MTF-C code by ØavioÒ\n" );
		G_sprint( self, 2, "Bug reports to avirox38@hotmail.com\n" );
		G_sprint( self, 2, "http://ÁíîåóéáÇáíåó®ãïí\n" );
		...
		//mtf
		TeamFortress_Alias( "ir", 50, 0 );		// night vision
