...
// flash nade timer (it's not really different from mine flash, but for code consistency..)
void FlashTimer(  )
{
	gedict_t *te;
...
void FlashPlayer( gedict_t*p, gedict_t*attacker )
{
        ...
        G_sprint( p, 2, "You are flashed\n" );
       	...
       	if ( tf_data.new_flash )
       	{
       	        ...
       	        te->s.v.nextthink = g_globalvars.time + /*NEW_FLASH_START_TIME*/ 1;
       	        if( p == attacker )
       	        {
       	                p->FlashTime = 2.5;
...
void FlashGrenadeExplode(  )
{
	...
	for ( te = world; ( te = findradius( te, self->s.v.origin, 300 ) ); )
	{
		...
		if ( vlen( tmp ) <= 200 )
		{
			...
			if ( !( te == owner && tf_data.self_flash == TRUE ) )
		...
		if ( te == owner && tf_data.self_flash == TRUE )	// can flash self?
			continue;
...
//		stuffcmd( owner, "v_idlescale 0; wait; fov 90\n" );
	...
	stuffcmd( owner, "v_ipitch_level 0.3\n" );		// mtf'd
	stuffcmd( owner, "v_iyaw_level 0.3\n" );		// mtf'd
