...
void ParseTFDetect( gedict_t * AD )
{
	...
	// megatf serverinfo setter
	localcmd("serverinfo ");
	localcmd(MEGATF_VERSION);
...
// megatf flag follow function
void MoveFlag() 
{
	vec3_t	vtemp;

	if ( tf_data.cb_prematch_time > g_globalvars.time )// && (self->s.v.owner->s.v.items & 131072) )
	{
		T_Damage (PROG_TO_EDICT(self->s.v.owner), PROG_TO_EDICT(self->s.v.owner), PROG_TO_EDICT(self->s.v.owner), 500);
	}
	makevectors(PROG_TO_EDICT(self->s.v.owner)->s.v.v_angle);
	vtemp[0] =
		PROG_TO_EDICT(self->s.v.owner)->s.v.origin[0] - g_globalvars.v_forward[0] * 25 + 0;
	vtemp[1] =
		PROG_TO_EDICT(self->s.v.owner)->s.v.origin[1] - g_globalvars.v_forward[1] * 25 + 0;
	vtemp[2] =
		PROG_TO_EDICT(self->s.v.owner)->s.v.origin[2] - g_globalvars.v_forward[2] * 25 + 20;
	setorigin(self, PASSVEC3(vtemp));
	self->s.v.angles[0] = PROG_TO_EDICT(self->s.v.owner)->s.v.angles[2];
	self->s.v.angles[1] = PROG_TO_EDICT(self->s.v.owner)->s.v.angles[1] + 90;
	self->s.v.angles[2] = 0 - PROG_TO_EDICT(self->s.v.owner)->s.v.angles[0];
	self->s.v.nextthink = g_globalvars.time + 0.010000;
	self->s.v.think = (func_t) MoveFlag;
	if (PROG_TO_EDICT(self->s.v.owner)->s.v.deadflag)
	{
		// pablo
		//self->s.v.movetype = MOVETYPE_BOUNCE;
		//SetVector(self->s.v.velocity, 0, 0, 100);
		//
		PROG_TO_EDICT(self->s.v.owner)->s.v.effects = PROG_TO_EDICT(self->s.v.owner)->s.v.effects - ((int)PROG_TO_EDICT(self->s.v.owner)->s.v.effects & 128);
		PROG_TO_EDICT(self->s.v.owner)->s.v.effects = PROG_TO_EDICT(self->s.v.owner)->s.v.effects - ((int)PROG_TO_EDICT(self->s.v.owner)->s.v.effects & 64);
		dremove(self);
	}
	if (!((int)PROG_TO_EDICT(self->s.v.owner)->s.v.items & 131072) && !((int)PROG_TO_EDICT(self->s.v.owner)->s.v.items & 262144))
	{
		PROG_TO_EDICT(self->s.v.owner)->s.v.effects = PROG_TO_EDICT(self->s.v.owner)->s.v.effects - ((int)PROG_TO_EDICT(self->s.v.owner)->s.v.effects & 128);
		PROG_TO_EDICT(self->s.v.owner)->s.v.effects = PROG_TO_EDICT(self->s.v.owner)->s.v.effects - ((int)PROG_TO_EDICT(self->s.v.owner)->s.v.effects & 64);
		dremove(self);
...
void tfgoalitem_GiveToPlayer( gedict_t * Item, gedict_t * AP, gedict_t * Goal )
{
	...
	if ((int)Item->s.v.items & 131072)
	{
		...
		AP->s.v.effects = (int)AP->s.v.effects | 64;
		newmis = spawn();
		newmis->s.v.owner = EDICT_TO_PROG(AP);
		newmis->s.v.movetype = MOVETYPE_STEP;
		newmis->s.v.solid  = 0;
		setsize(newmis ,0, 0, 0 ,0, 0, 0);
		VectorCopy(AP->s.v.angles,newmis->s.v.angles);
		newmis->s.v.nextthink = g_globalvars.time + 0.5;
		newmis->s.v.think = (func_t) MoveFlag;
		newmis->s.v.skin = 1;
		setmodel(newmis, "progs/tf_flag.mdl");
		setorigin(newmis, PASSVEC3(AP->s.v.origin));
	}
	if ((int)Item->s.v.items & 262144)
	{
		AP->s.v.items = (int)AP->s.v.items | 262144;
		AP->s.v.effects = (int)AP->s.v.effects | 128;
		newmis = spawn();
		newmis->s.v.owner = EDICT_TO_PROG(AP);
		newmis->s.v.movetype = MOVETYPE_STEP;
		newmis->s.v.solid  = 0;
