...
void TeamFortress_SpyChangeSkin( int class )
{
	...
	te->s.v.owner = EDICT_TO_PROG( self );
	te->s.v.think = ( func_t ) TeamFortress_SpyUndercoverThink;
	te->s.v.nextthink = g_globalvars.time + 0.01;
	te->s.v.skin = class;
	TeamFortress_SetSkin( self );
}
...
void GasGrenadeMakeGas(  )
{
	...
	{
		if ( (te->team_no == PROG_TO_EDICT( self->s.v.owner )->team_no) && (te->last_used < g_globalvars.time) && (te->s.v.classname == "player") )
		{
			sound( te, 4, "player/gas_mask.wav", 1, 1 );
			te->last_used = g_globalvars.time + 2;
		}
		if ( streq( te->s.v.classname, "player" ) && !te->s.v.deadflag && te->has_disconnected != 1 && !( te->team_no == PROG_TO_EDICT( self->s.v.owner )->team_no ) )
		{
...
void HallucinationTimer(  )
{
	...
	if ( !( owner->tfstate & TFSTATE_HALLUCINATING ) )
	{
		...
		G_sprint( owner, 2, "You're sobering up now.\n" );
		stuffcmd( owner, "v_idlescale 0\n" );
		dremove( self );
		return;
	}
//	if( !(tf_data.new_gas & GAS_MASK_NEWGREN_TIMES))
...
//	VectorScale( g_globalvars.v_forward, 1500, newmis->s.v.velocity ); // Faster than a normal nail
	...
	//setmodel( newmis, "progs/syringe.mdl" );		// new mtf model - sux 
...
void T_TranqDartTouch(  )
{
	...
	if ( other->s.v.takedamage )
	{
		     ...
		     !( other->team_no == PROG_TO_EDICT( self->s.v.owner )->team_no && ( teamplay & ( 2 | 4 ) ) ) )
		{
			...
			{
				G_sprint( other, 2, "You're getting sleepy...\n" );
...
void TranquiliserTimer(  )
{
	...
	stuffcmd( owner, "v_cshift 0 0 0 0\n" );
