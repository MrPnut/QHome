...
/*	if ( g_globalvars.parm15 )
	...
	if ( !( tf_data.toggleflags & TFLAG_FIRSTENTRY ) )
	{
		...
		// add this feature in later - xavior
...
////top colors
		...
		GetSVInfokeyString( "self_flash", NULL, st, sizeof( st ), "on" );
		if ( !strcmp( st, "on" ) )
			tf_data.self_flash = 1;
		else
			tf_data.self_flash = 0;

		GetSVInfokeyString( "exec_class", NULL, st, sizeof( st ), "on" );
		if ( !strcmp( st, "off" ) )
			tf_data.exec_class = 0;
		else
			tf_data.exec_class = 1;
		...
		// mtf server vars
		GetSVInfokeyString( "coloredlights", "clights", st, sizeof( st ), "on" );
		if ( !strcmp( st, "off" ) )
			tf_data.coloredlights = 0;
		else
			tf_data.coloredlights = TRUE;
		//footsteps
		GetSVInfokeyString( "footsteps", "fteps", st, sizeof( st ), "on" );
		if ( !strcmp( st, "off" ) )
			tf_data.footsteps = 0;
		else
			tf_data.footsteps = TRUE;
		//use old (tf2.9) obituary?
		/*GetSVInfokeyString( "old_death", "odeath", st, sizeof( st ), "on" );
		if ( !strcmp( st, "off" ) )
			tf_data.old_dmsg = 0;
		else
			tf_data.old_dmsg = TRUE;*/
		GetSVInfokeyString( "old_death", "odeath", st, sizeof( st ), "off" );
		if ( !strcmp( st, "on" ) )
			tf_data.old_dmsg = TRUE;
		// enable/disable drops
		GetSVInfokeyString( "drop1", NULL, st, sizeof( st ), "on" );
		if ( !strcmp( st, "off" ) )
			tf_data.drop1 = 0;
		else
			tf_data.drop1 = TRUE;
		GetSVInfokeyString( "drop2", NULL, st, sizeof( st ), "on" );
		if ( !strcmp( st, "off" ) )
			tf_data.drop2 = 0;
		else
			tf_data.drop2 = TRUE;
		GetSVInfokeyString( "drop3", NULL, st, sizeof( st ), "on" );
		if ( !strcmp( st, "off" ) )
			tf_data.drop3 = 0;
		else
			tf_data.drop3 = TRUE;
		// airscout/jetpack
		GetSVInfokeyString( "airscout", NULL, st, sizeof( st ), "on" );
		if ( !strcmp( st, "off" ) )
			tf_data.airscout = 0;
		else
			tf_data.airscout = TRUE;
		// old nail gren
		GetSVInfokeyString( "old_nail", "onail", st, sizeof( st ), "off" );
		if ( !strcmp( st, "on" ) )
			tf_data.old_nail = TRUE;
...
void PutClientInServer()
{
	...
	char    s[10];
	// version stuff
	static char client_ver[32][128]; // client name
	int ctype; // mqw client type ID
...
// oh, this is a hack!
	...
	// client version recognition code
	//  Should this maybe be put in ClientConnect? - xav
	self->client = "unknown";
	ctype = 0;
	infokey( self, "*client", *client_ver, sizeof( *client_ver ) );
	if ( !strcmp( *client_ver, "mqwcl 0.96" ) )
		ctype = 1;
	if ( !strcmp( *client_ver, "mqwcl 0.95" ) )
		ctype = 2;
	if ( !strcmp( *client_ver, "mqwcl 0.93" ) )
		ctype = 3;
	if ( ctype > 0 )
	{
		self->client = "mqwcl";
	...
	infokey( self, "*z_ver", *client_ver, sizeof( *client_ver ) );
	if ( strcmp( *client_ver, "" ) )
	{
		self->client = "zquake";
		self->clientv = ( string_t ) client_ver;
	}
	infokey( self, "*FuhQuake", *client_ver, sizeof( *client_ver ) );
	if ( strcmp( *client_ver, "" ) )
	{
		self->client = "fuhquake";
		self->clientv = ( string_t ) *client_ver;
	}
	if ( !strcmp( self->client, "unknown" ) ) // unknown client, so lets just read the raw version info
	{
		//GetInfokeyString( self, "ch", "classhelp", value, sizeof( value ), NULL)
		//infokey( self, "*ver", *client_ver, sizeof( *client_ver ) );
		self->client = &(client_ver[NUM_FOR_EDICT(self)-1][0]);
		infokey( self, "*ver", self->client, 128 );
		if ( strcmp( *client_ver, "" ) )
			self->client = *client_ver;
		else {
			self->client = &(client_ver[NUM_FOR_EDICT(self)-1][0]);
			infokey( self, "*client", self->client, 128 );
			if ( strcmp( *client_ver, "" ) )
				self->client = *client_ver;
		}
	}
	...
	if ( !self->playerclass )
	{
		// print client version here
		if ( strcmp( self->clientv, "" ) )
			G_bprint( 2, "%s is using %s %s\n", self->s.v.netname, self->client, self->clientv );
		else
			G_bprint( 2, "%s is using %s\n", self->s.v.netname, self->client );
...
//      vec3_t  end;
	...
	// heguy post-20mm anti-speedbug
	if ( self->nojumptime > g_globalvars.time )
	{
		if ( self->s.v.velocity[0] > 70 )
			self->s.v.velocity[0] = 70;
		else
		{
			if ( self->s.v.velocity[0] < -70 )
				self->s.v.velocity[0] = -70;
		}
		if ( self->s.v.velocity[1] > 70 )
			self->s.v.velocity[1] = 70;
		else
		{
			if ( self->s.v.velocity[1] < -70 )
				self->s.v.velocity[1] = -70;
...
//		self->s.v.velocity[2] += 130;
	self->s.v.flags = ( int ) self->s.v.flags - ( ( int ) self->s.v.flags & FL_JUMPRELEASED );
	self->s.v.button2 = 0;
	if (self->playerclass == PC_SPY) {
		self->do_spyjump = g_globalvars.time + 1.25;		// seems to be the lesser of the 2 evil ways to do mtf spy jump physics :x
		sound( self, 4, "hknight/slash1.wav", 0.7, 1 );
	}
	else
...
//      vec3_t  src;
	...
	if ( !self->is_feigning )
	{
		if ( self->do_spyjump != 0 && self->do_spyjump > g_globalvars.time ) {
			self->s.v.velocity[2] += 130;
			self->do_spyjump = 0;
		}
		if ( self->s.v.button2 ) {
			if ( ( ( int ) self->s.v.flags & FL_ONGROUND ) )
				o_g = 1;
			PlayerJump();
			//if ( !self->s.v.waterlevel && self->playerclass == PC_SPY && o_g == 1 )
			//	self->do_spyjump = TRUE;
		}
		else
			self->s.v.flags = ( int ) self->s.v.flags | FL_JUMPRELEASED;
	} else
	...
	if ( self->jump_flag < -300 && ( ( int ) self->s.v.flags & FL_ONGROUND ) && self->s.v.health > 0 )
	{
		...
		else
		{
			if ( self->jump_flag < -650 )
			{
				...
				if ( self->playerclass == PC_SPY )
					sound( self, 2, "hknight/hit.wav", 1, 1 );
				else
...
//      float   fr;
	...
	sound( self, CHAN_BODY, "misc/eject.wav", 1, ATTN_NONE );
...
void ClientObituary( gedict_t * targ, gedict_t * attacker )
{
	...
	if ( tf_data.old_dmsg != TRUE || tf_data.megatf_dmg == tf_data.deathmsg ) {
		Mega_ClientObituary( targ, attacker );
		return;
	}
