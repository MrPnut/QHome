...
//mtf
void	Menu_Yell_Input( int inp );
void	Menu_Taunt_Input( int inp );
void	Menu_Taunt( menunum_t menu );
void	Menu_Yell( menunum_t menu );
void	Menu_DropStuff( menunum_t menu );
void	Menu_DropStuff_Input( int inp );
...
const static menu_t menus[] = {
	...
	{-1, 0, 0, Menu_Yell, Menu_Yell_Input},	//MENU_YELL,
	{-1, 0, 0, Menu_Taunt, Menu_Taunt_Input},	//MENU_TAUNT,
	{-1, 0, 0, Menu_DropStuff, Menu_DropStuff_Input},	//MENU_DROPSTUFF,
...
void Menu_ClassHelp( menunum_t menu )
{
	...
	infokey( self, "tf_help", usehelp, sizeof( usehelp ) );
	if ( !strcmp( usehelp, "off" ) )
		return;
...
void Menu_Engineer_Input( int inp )
{
	...
	case 1:
		...
		{
			sound (self, 3, "items/repair.wav", 1, 1);
...
void Menu_EngineerFix_Dispenser_Input( int inp )
{
	...
	case 2:
		...
		sound (self, 3, "weapons/tnkatck4.wav", 1, 1);
		break;
	case 3:
		metalcost =
...
void Menu_EngineerFix_SentryGun_Input( int inp )
{
	...
	case 2:
				  ...
				  || tg_data.tg_enabled ) )
			{
				...
				{
					sound( self->building, 3, "weapons/tnkatck4.wav",
					       1, 1 );
	...
	case 7:
		if ( self->building->s.v.weapon == 3 )
		{
			self->s.v.ammo_rockets += self->building->s.v.ammo_rockets;
			if ( self->s.v.ammo_rockets > self->maxammo_rockets )
				self->s.v.ammo_rockets = self->maxammo_rockets;
			self->building->s.v.ammo_rockets = 0;
			G_sprint( self, 2, "You have unloaded your sentry's rocket cache.\n");
		}
		else
			return;
/*		if ( !tg_data.tg_enabled )
...
void Menu_Dispenser_Input( int inp )
{
	...
	case 1:
		...
		self->building->s.v.ammo_cells = self->building->s.v.ammo_cells - am;
		self->s.v.ammo_cells = self->s.v.ammo_cells + am;
		sound (self, 3, "weapons/rocklr1a.wav", 1, 1);
		break;
	case 2:
	...
	case 3:	//heal
		...
		{
			G_sprint( self, 2, "No vaccine has been loaded by Medic\n" );
		}
		break;
	case 4:
		if ( self->building->s.v.skin == 1 )
		{
			...
			self->rad_time = 1;
			self->radsuit_finished = g_globalvars.time + 30;
			G_sprint( self, 2, "You have a BioSuit\n" );
		}
		else
		{
			G_sprint ( self, 2, "No BioSuit has been loaded by Medic\n" );
		}
		break;
	default:
		return;
		...
		else
		{
			...
			else
			{
				...
				else
				{
					if (inp == 5)
					{
						sound(self, 1, "speech/call911.wav", 1, 1);
					}
					else
					{
						if (inp == 6)
						{
							sound(self, 1, "speech/kicknads.wav", 1, 1);
						}
						else
						{
							if (inp == 7)
							{
								sound(self, 1, "speech/idoasskk.wav", 1, 1);
							}
							else
							{
								if (inp == 8)
								{
									sound(self, 1, "speech/pathetic.wav", 1, 1);
								}
								else
								{
									if (inp > 8)
...
void Menu_Yell_Input( int inp )
{
	...
	if (inp == 1)
	{
		sound(self, 1, "speech/yell/coverme.wav", 1, 1);
	}
	else
	{
		if (inp == 2)
		{
			sound(self, 1, "speech/yell/overhere.wav", 1, 1);
		}
		else
		{
			if (inp == 3)
			{
				sound(self, 1, "speech/yell/thisway.wav", 1, 1);
			}
			else
			{
				if (inp == 4)
				{
					sound(self, 1, "speech/yell/followme.wav", 1, 1);
				}
				else
				{
					if (inp == 5)
					{
						sound(self, 1, "speech/yell/incomng.wav", 1, 1);
					}
					else
					{
						if (inp == 6)
						{
							sound(self, 1, "speech/yell/holdup.wav", 1, 1);
						}
						else
						{
							if (inp == 7)
							{
								sound(self, 1, "speech/yell/gogogo.wav", 1, 1);
							}
							else
							{
								if (inp == 8)
								{
									sound(self, 1, "speech/yell/lookout.wav", 1, 1);
								}
								else
								{
									if (inp == 9)
									{
										sound(self, 1, "speech/yell/firehole.wav", 1, 1);
									}
									else
									{
										if (inp > 9)
