...
////top colors
		...
		// join music server toggle
		GetSVInfokeyString( "tf_music", "mu", st, sizeof( st ), "on" );
		if ( !strcmp( st, "off" ) )
			tf_data.tf_music = 0;
		else
			tf_data.tf_music = TRUE;
...
// oh, this is a hack!
	...
	self->client = "unknown";
	ctype = 0;
	infokey( self, "*client", cl, sizeof( cl ) );
	if ( !strcmp( cl, "mqwcl 0.96" ) )
		ctype = 1;
	if ( !strcmp( cl, "mqwcl 0.95" ) )
		ctype = 2;
	if ( !strcmp( cl, "mqwcl 0.93" ) )
		ctype = 3;
	if ( ctype > 0 )
	{
		self->client = "mqwcl";
	...
	infokey( self, "*z_ver", cl, sizeof( cl ) );
	if ( strcmp( cl, "" ) )
	{
		ctype = 4;
		self->client = "zquake";
		self->clientv = ( string_t ) cl;
	}
	infokey( self, "*FuhQuake", cl, sizeof( cl ) );
	if ( strcmp( cl, "" ) )
	{
		ctype = 4;
...
//spawn_tdeath после установки размеров игрока
	...
	if ( !strneq( self->s.v.noise, "player/mdeath5.wav" ) )
	{
		sound( self, 2, "player/malive5.wav", 1, 1 );
	}
}
...
//      vec3_t  src;
	...
	{
		if ( intermission_running && self->last_used != 999 )
		{
			if ( g_random() < 0.5 )
			{
