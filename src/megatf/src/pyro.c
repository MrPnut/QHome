...
void W_FireFlame(  )
{
	...
	if ( self->playerclass != PC_HVYWEAP )
		...
		VectorScale( flame->s.v.velocity, 100, flame->s.v.velocity );

	flame->s.v.touch = ( func_t ) Flamer_stream_touch;
//	flame->s.v.think = ( func_t ) s_explode1;
	flame->s.v.think = ( func_t ) s2_explode1;
	flame->s.v.effects = 8;
	if ( self->s.v.classname == "airstrike" )
	{
		flame->s.v.velocity[0] = 0;
		flame->s.v.velocity[1] = 0;
...
void W_FireIncendiaryCannon(  )
{
	...
	sound( self, 1, "doors/airdoor2.wav", 1, 1 );
	KickPlayer( -3, self );
	newmis = spawn(  );
	...
	VectorScale( newmis->s.v.velocity, 750, newmis->s.v.velocity );		// maybe this val will change
