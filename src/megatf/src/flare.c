...
void FlareGrenadeTouch(  )
{
	...
	if ( trap_pointcontents( PASSVEC3( self->s.v.origin ) ) == CONTENT_SKY )
	{
		...
		setorigin ( newmis, ( self->s.v.origin[0] - 0 ), ( self->s.v.origin[1] - 0 ), ( self->s.v.origin[2] - 30 ) );
		newmis->s.v.movetype = 8;
		newmis->s.v.solid = 1;
		newmis->s.v.effects = 4;
		newmis->s.v.effects = ( int ) newmis->s.v.effects + 1;
		sound (newmis, 3, "items/flare1.wav", 1, 1);
		newmis->s.v.velocity[2] = -30;
		newmis->s.v.velocity[0] = ( g_random(  ) * 10 );
		newmis->s.v.velocity[1] = ( g_random(  ) * 10 );
		newmis->s.v.nextthink = g_globalvars.time + 30;
		newmis->s.v.think = ( func_t ) SUB_Remove;
		newmis->s.v.touch = ( func_t ) SUB_Remove;
		newmis->s.v.classname = "flare";
		dremove( self );
		//return;
	}
	if ( other == world )
	{
		...
		self->s.v.movetype = 6;			// 6 in megatf
		sound(self, 1, "weapons/bounce.wav", 1, 1);
	}
